    ...
    <main className='grid sm:grid-cols-1 md:grid-cols-2 gap-12 lg:grid-cols-3'>
      {filtered.map((variant) => (
        <Link
          className='py-2'
          key={variant.id}
          href={`/products/${variant.id}?id=${variant.id}&productID=${variant.productID}&price=${variant.product.price}&title=
          ${variant.product.title}&type=${variant.productType}&image=${variant.variantImages[0].url}`}
        >
          <Image
